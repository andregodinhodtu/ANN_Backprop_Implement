...
def make_random_batch(seed, n_samples, n_features=4, n_outputs=2):
    ...
    rng = random.Random(seed)
    X = [
        [[rng.gauss(0, 1)] for i in range(n_features)]
        for j in range(n_samples)
    ]
    Y = [
        [[float(rng.randint(0, 1))] for i in range(n_outputs)]
        for j in range(n_samples)
    ]
    return X, Y
...
def test_compute_loss_returns_float():
    ann = make_ann()
    X = [[[1.0]] * 4 for i in range(3)]   # 3 samples, each (4, 1)
    Y = [[[0.0]] * 2 for i in range(3)]   # 3 targets, each (2, 1)
...
def test_compute_loss_is_non_negative():
    ...
    # BCE Loss is always bigger than 0
    ann = make_ann()
...
def test_compute_loss_single_vs_batch_consistency():
    ...
    # Asserting that the batch response of the compute loss is the same
    # as averaging the batch
    batched = ann.compute_loss(X, Y)
    per_sample = [ann.compute_loss([X[i]], [Y[i]]) for i in range(len(X))]

    assert batched == pytest.approx(sum(per_sample)/len(per_sample))
